...
def driver(request):
    ...
    options = Options()
    # options.add_argument("--headless")  # Запускк Google Chrome в фоновом режиме
...
def prepair_data(driver):
    ...
    auth_page.login_input.send_keys('al3xsaburov@gmail.com')
    auth_page.password_input.send_keys('parolTest666')
...
@pytest.mark.parametrize('login, password', [
    ('al3xsaburov@gmail.com', 'parolTest666'),
    ('900 992 67 70', 'parolTest666')
])
...
def test_delete_patronymic(driver):
    ...
    # Добавляем отчество
    page_id_authorized.edit_name_btn.click()
    page_id_authorized.fathers_name_input.send_keys('Тестотчество')
    page_id_authorized.save_btn.is_clickable()
    page_id_authorized.save_btn.click()
...
def test_error_massage_if_invalid_pass(driver):
    ...
    page.login_input.send_keys('12345@mail.ru')
    page.password_input.send_keys('123456')
    page.enter_btn.click()
